...
- ユーザーが「git commit」と言った場合は、即座にコミットを実行してください
- このルールは絶対に遵守し、常に従わなければなりません

**GITコミットポリシー - 重要ルール**
- **絶対に`git add`を実行してはいけない - 全プロジェクト共通**
- **ユーザーが「git commit」と言った時、絶対に`git add`を実行しない** ステージングされている変更のみでコミットする
...
- ステージング済みと未ステージングの変更が混在している場合、それはユーザーの意図的な選択と判断し、確認せずにステージング済みの変更のみでコミットを実行してください
- ステージングされたファイルがない場合は、コミットする変更がないことをユーザーに伝えてください
- `git add`、`git add -A`、`git add .`、`git add -u`など、あらゆる形式の`git add`コマンドを禁止します
- これは交渉の余地のないルールで、正確に従わなければなりません
- **`git commit`実行前は必ず`git status`で確認する**
- コミット可能な変更があるかどうかを事前に確認してからコミットを実行する
- **`git status --short` と `git status --porcelain` の使用を禁止する**
- 1列目と2列目の `M` / ` M` を誤読するリスクが高い
- 常に `git status` の通常出力を使用すること
- **コミットメッセージ作成時の必須手順**
- コミットメッセージを作成する前に、必ず`git diff --cached`でステージングされた変更の詳細内容を確認する
- ブランチ名やファイル名からの推測ではなく、実際の変更内容に基づいてコミットメッセージを作成する
- ステージングされていない変更内容を含めたコミットメッセージは絶対に作成しない
- 実際の変更内容と一致しないコミットメッセージは絶対に作成しない

**GitHub PR/Issueポリシー - 必須ルール**
- **PRやIssueの作成・編集時は必ず以下の署名を含める**
- PR本文、Issue本文、コメントの最後に必ず以下の一文を追加する：
  ```
  🤖 Generated with [Claude Code](https://claude.ai/code)
  ```
- この署名は省略してはいけない
- PR作成時（`gh pr create`）は本文の最後に必ず含める
- PR編集時（`gh pr edit`）は本文の最後に必ず含める
- Issue作成時（`gh issue create`）は本文の最後に必ず含める
- Issue編集時（`gh issue edit`）は本文の最後に必ず含める
- PRやIssueへのコメント時（`gh pr comment`、`gh issue comment`）も必ず含める
- これは絶対に遵守すべきルールで、例外なく適用される
- この署名により、AIアシスタントが作成したコンテンツであることを明確にする
...
- 余計なエントリを追加しない
- **パスの使い分け:**
  - **絶対パス（`/`始まり）**: ルートディレクトリ固有のファイル
    - 例: `/.env`, `/vendor`, `/node_modules`, `/public/build`
  - **相対パス**: どのディレクトリでも発生しうるOSファイル
    - 例: `.DS_Store`, `Thumbs.db`
- カテゴリごとにコメントで整理する
...
- **プライベートデータとパブリックデータの区別**
  - Git追跡ファイル（変更可能）とユーザーのプライベートファイル（許可なしに触らない）を常に区別する
  - 変更前に必ず`git status`と`git ls-files`で追跡状況を確認する
- **機密情報の保護**
  - コードや追跡ファイルに個人情報（URL、名前、トークン）をハードコードしない
  - テンプレートファイル（`.env.example`、`.sample.md`）のみ変更可能
  - 機密性の高い個人情報・業績データを含むファイルは慎重に扱い、内容を外部に漏洩させない
...
**コードスタイル**
- **既存のコメントを勝手に削除しないでください**
  - コード変更時に、変更対象外のコメントは絶対に触らない
  - コメントの内容が変更後のコードと矛盾する場合は、削除ではなく修正する
  - コメント削除はユーザーの明示的な指示がある場合のみ
- ファイルを変更する際は、まずそのファイルのコード規約を理解してください
- コードスタイルを模倣し、既存のライブラリとユーティリティを使用し、既存のパターンに従ってください
- **ソースコードのコメントはすべて日本語で書く**
- **例外**: 技術的な固有名詞・サービス名は英語のまま使用すること
  - 例: `# TKE Managed Cluster`, `# NAT Gateway`, `# AWS Lambda`
  - 理由: 公式ドキュメントやコンソールと一致させるため

**コメント記述ポリシー - 絶対ルール**
- **無駄なコメント、意味不明なコメントは絶対に残さない**
- 冗談めいた表現、比喩的な表現、感情的な表現は使用しない
- 技術的事実のみを簡潔に記述する
- コメントは必要最小限にとどめ、自己説明的なコードを書く
- **禁止される表現の例：**
  - 「〜を駆逐」「〜を撲滅」などの過激な表現
  - 「〜かもしれない」「〜だろう」などの曖昧な表現
  - ユーモアや皮肉を含む表現
...
- **許可されるコメントの例：**
  ...
  - 設定値の説明：「# RDS接続用の変数」
  - セクション区切り：「# terraform」「# release」

**ドキュメント・コメント・ヘルプ記述ポリシー - 絶対ルール**
- **括弧書きの補足説明を入れない**
- Makefileのヘルプ、コミットメッセージ、ドキュメントなど全てに適用される
- 必要最小限の情報のみを記述し、余計な説明は一切追加しない
- **悪い例：**
  - `path: ## $PATHを見やすく表示（先頭2スラッシュでグループ化）`
  - `Add user authentication (using JWT tokens)`
  - `データベース接続を追加（PostgreSQL使用）`
- **良い例：**
  - `path: ## $PATHを見やすく表示`
  - `Add user authentication`
  - `データベース接続を追加`
- 実装の詳細や方法は、コードを見ればわかるため記述不要
- シンプルで簡潔な表現を常に心がける

**人名表記ポリシー - 絶対ルール**
- **漢字表記が確実にわかる場合のみ漢字を使用する**
- ドキュメント作成時、人名の漢字が既出・確認済みの場合のみ漢字表記を使用
- 漢字表記が不明な場合は、推測せずアルファベット表記（ローマ字）を使用する
- **悪い例：**
  - 「石川正幸」（Masayuki Ishikawaとしか記載がない場合）
- **良い例：**
  - 「Masayuki Ishikawa」（漢字が確認できない場合）
  - 「石川将行」（「石川将行」と記載がある場合）
- 人名の推測による漢字変換は絶対に行わない
...
- **SSH/RDSアクセスはSession Managerを優先**
  - EC2やRDSへのアクセスにはSSHゲートウェイではなくSession Managerを推奨する
  - セキュリティグループでSSHポート（22）を開放する必要がなくなる
  - アクセスログがCloudTrailに記録され、監査性が向上する
- **標準的なSession Managerコマンド**
  - `make ssm-port-forward` - RDSへのポートフォワーディング
  - `make ssm-connect` - EC2へのシェルアクセス
  - `make ssm-list-instances` - 利用可能なインスタンス一覧表示
- **必須の前提条件**
  - AWS CLI v2がインストールされていること
  - Session Manager Pluginがインストールされていること（`brew install --cask session-manager-plugin`）
  - EC2インスタンスにSSM Agentがインストール済みであること
  - IAMロール/ユーザーに適切なSSM権限が付与されていること
...
**シェルコマンドポリシー - 絶対ルール**
- **`rm`コマンドは必ず`rm -f`を使用する**
  - ユーザー環境では`rm`が`rm -i`にエイリアスされているため、対話的な確認プロンプトが発生しBashツールがハングする
  - ファイル削除時は必ず`rm -f`または`rm -rf`を使用すること
  - `rm`を単体で使用することは絶対に禁止
  - これは交渉の余地のないルールで、正確に従わなければならない
