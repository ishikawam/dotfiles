...
######## section: xcode-select ##################################################################

xcode-select-section() {
    head "0. xcode-select"
...
mas-section() {
    ...
    for adam_id in "${!mas_apps[@]}"; do
        local app_name="${mas_apps[$adam_id]}"
        local result
...
install-font() {
    ...
    local ICLOUD_FONTS=~/Library/Mobile\ Documents/com~apple~CloudDocs/Fonts
...
install-picasa() {
    ...
    local ONEDRIVE=~/Library/CloudStorage/OneDrive-Personal
...
create-local-apps() {
    ...
    local KILL_NOTIFICATION_APP="$HOME/Applications/kill notification.app"
    local KILL_NOTIFICATION_SCRIPT='do shell script "killall usernoted NotificationCenter 2>/dev/null; sleep 0.5; sqlite3 ~/Library/Group\\ Containers/group.com.apple.usernoted/db2/db \"DELETE FROM record; DELETE FROM delivered; DELETE FROM displayed; DELETE FROM requests;\"; killall usernoted NotificationCenter 2>/dev/null; true"'
    local KILL_NOTIFICATION_HASH
    KILL_NOTIFICATION_HASH=$(echo "$KILL_NOTIFICATION_SCRIPT" | md5)
    local KILL_NOTIFICATION_HASH_FILE="$HOME/Applications/kill notification.app.md5"
    ...
    local EJECT_ALL_APP="$HOME/Applications/eject all.app"
    local EJECT_ALL_SCRIPT='set failedList to {}
...
if failedList is not {} then
    ...
    local EJECT_ALL_HASH
    EJECT_ALL_HASH=$(echo "$EJECT_ALL_SCRIPT" | md5)
    local EJECT_ALL_HASH_FILE="$HOME/Applications/eject all.app.md5"
...
case "${1:-all}" in
    all)               run-all ;;
    xcode-select)      xcode-select-section ;;
    rosetta)           rosetta ;;
    homebrew)          homebrew ;;
    asdf)              asdf-section ;;
    mas)               mas-section ;;
    ...
    font)              install-font ;;
    picasa)            install-picasa ;;
    create-local-apps) create-local-apps ;;
    other-tools)       other-tools ;;
    *)
        echo "Usage: $0 [all|xcode-select|rosetta|homebrew|asdf|mas|hostname|font|picasa|create-local-apps|other-tools]"
        exit 1
        ;;
esac
